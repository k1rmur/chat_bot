...
from config_data.config import db_url, token
...
from vkbottle.bot import Bot, BotLabeler, Message, MessageEvent
...
async def send_intro_message(message: Message):
    ...
    tasks = [
        message.ctx_api.messages.send(
            peer_id=chat_id, message=text, random_id=random.randint(1, 1e6)
        )
    ...
    await message.ctx_api.messages.send(
        peer_id=322077458,
        message=f"Прошла рассылка сообщения:\n\n{text}",
...
async def ask_for_rating(message: Message):
    ...
    tasks = [
        message.ctx_api.messages.send(
            peer_id=chat_id,
            message=text,
            keyboard=inline_rating_keyboard(),
            random_id=random.randint(1, 1e6),
        )
        for chat_id in result
    ]
...
async def process_rating(event: MessageEvent, bot: Bot):
    ...
    await bot.api.messages.delete(
        peer_id=event.peer_id, message_ids=[event.message_id], delete_for_all=True
    )
...
async def process_start_command(message: Message):
    ...
    logger.info(
        f"Пользователь {message.from_id} начал диалог, код чата {message.peer_id}"
    )
...
async def send(message: Message):
    ...
    if text in LEXICON_RU:
        ...
        try:
            ...
            logger.info(
                f'Пользователь {message.from_id} задал вопрос: "{text}", получен ответ: "{answer}"'
            )
            ...
            await bot.api.messages.send(
                peer_id=200820242, message=error_text, random_id=random.randint(1, 1e6)
            )
