...
QA_PROMPT_STR = (
    ...
    "{context_str}\n"
    '"""\n'
    "Используя в первую очередь информацию из контекста, а во вторую очередь, свои знания "
    "ответь на вопрос: {query_str}\n. Не сообщай имя файла, откуда взял информацию и не добавляй лишних коментариев, например 'в контексте указано, что'. Если в контексте нет ответа или человек задаёт слишком высокоуровневый запрос, объясни, что ты не можешь отвечать на такие вопросы, а разработан для ответов на более конкретные вопросы."
)
