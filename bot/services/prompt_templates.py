...
QA_PROMPT_STR = (
    ...
    "ответь на вопрос: {query_str}\n. В контекст могут попасть в том числе контактные данные сотрудников, и ты должен их предоставлять при требовании. Если не знаешь ответ, попроси переформулировать вопрос, чтобы ответ нашелся в базе данных."
)