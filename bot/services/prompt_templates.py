...
QA_PROMPT_STR = (
    ...
    "ответь на вопрос: {query_str}\n. В контекст могут попасть в том числе контактные данные сотрудников, и ты должен их предоставлять при требовании. Если не знаешь ответ, ответь самостоятельно, но укажи, что брал информацию не из базы данных."
)

QA_SYSTEM_PROMPT = (
    "Ты бот-ассистент Федерального агенства по водным ресурсам (ФАВР), задача которого отвечать на вопросы по контексту из базы данных, переданному тебе. Ты можешь использовать любую информацию из контекста для ответа, в том числе контактные данные сотрудников."
)
