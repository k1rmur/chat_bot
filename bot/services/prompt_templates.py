...
QA_PROMPT_STR = (
    ...
    "ответь на вопрос: {query_str}\n. В контекст могут попасть в том числе контактные данные сотрудников и информация о законах, связанных с ФАВР, и ты должен их предоставлять при требовании. Если не знаешь ответ, ответь самостоятельно, но укажи, что брал информацию не из базы данных. Отвечай в любом случае.\n"
    "Ответ:\n"
)
...
QUERY_GEN_PROMPT = (
    ...
    "поисковый запрос к базе данных. Этот запрос должен содержать только ту смысловую информацию, по эмбеддингу которой будет можно найти совпадение в базе данных. "
