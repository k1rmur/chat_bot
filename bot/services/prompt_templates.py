...
QA_PROMPT_STR = (
    ...
    "ответь на вопрос: {query_str}\n. Не сообщай имя файла, откуда взял информацию и не добавляй лишних коментариев, например 'в контексте указано, что'. Если в контексте нет ответа или человек задаёт слишком высокоуровневый запрос, попроси уточнить запрос и не использовать общие формулировки."
)
