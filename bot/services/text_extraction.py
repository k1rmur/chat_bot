...
async def extract_news_from_document(document: Document, bot: Bot):
    ...
    facts = "\n".join([par.text for par in document.paragraphs[indices[0]:indices[1]]])
    world = "\n".join([par.text for par in document.paragraphs[indices[1]:indices[2]]])
    conferences = "\n".join([par.text for par in document.paragraphs[indices[2]:indices[3]]])
