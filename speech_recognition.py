...
def _parse_result(json_str: str) -> str:
    ...
    try:
        ...
        all_text_parts = []
        for segment in data: # Итерируемся по всем сегментам в data
            if 'results' in segment and segment['results']:
                # Возможно, и внутри results может быть массив
                for res_item in segment['results']:
                     if 'normalized_text' in res_item:
                        all_text_parts.append(res_item['normalized_text'])
                     # Также проверьте 'alternatives' если hypotheses_count > 1
                     elif 'alternatives' in res_item and res_item['alternatives']:
                         # Берем первую и самую уверенную гипотезу
                         if 'normalized_text' in res_item['alternatives'][0]:
                             all_text_parts.append(res_item['alternatives'][0]['normalized_text'])
...
class Audio:
    ...
    class Transcriptions:
        ...
        async def create(
            ...
            config: SpeechRecognitionConfig | None = None, # РЇРІРЅРѕ РїСЂРёРЅРёРјР°РµРј config
            **kwargs  # pylint: disable=unused-argument # РћСЃС‚Р°РІР»СЏРµРј РґР»СЏ СЃРѕРІРјРµСЃС‚РёРјРѕСЃС‚Рё РёР»Рё Р±СѓРґСѓС‰РёС… РїР°СЂР°РјРµС‚СЂРѕРІ
            ...
            Args:
                ...
                config: SpeechRecognitionConfig instance for custom settings
            ...
            try:
                ...
                task = await asyncio.to_thread(
                    ...
                    channels_count=channels_count,
                    language=language,
                    config=config  # РџРµСЂРµРґР°РµРј config
                )
